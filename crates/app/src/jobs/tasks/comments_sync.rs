...
use crate::jobs::tasks::feed_index::{base_url_from_feed, parse_search_index_entries, SearchIndexEntry};
...
async fn fetch_posts(state: &AppState) -> Result<Vec<PostRef>, JobError> {
    ...
    let entries = parse_search_index_entries(&body).map_err(|err| JobError::Comments(err.to_string()))?;
...
fn build_discussion_body(post: &PostRef) -> String {
    ...
    lines.push(format!("Auto-generated discussion for {url}", url = post.url));
    ...
    lines.push(format!("Path: `{}`", post.post_id));
    lines.push(String::new());
    lines.push("Generated by Inkstone.".to_string());
    lines.join("\n")
}
...
#[cfg(test)]
mod tests {
    use super::{path_from_url, slug_from_path, title_candidates, SearchIndexEntry};
