...
- `inkstone_token` (required; legacy `path` accepted during rollout): signed token containing the page path
- `path` (legacy): blog path, e.g. `/posts/hello/`
...
`PUT /v2/kudos` (or `POST /v2/kudos` for preflight-friendly clients)
